...
export const metadata: Metadata = {
  title: 'Bio Link',
  description:
    'AI Systems & Digital Products - Hubungi kami, kunjungi website utama, dan akses asisten cerdas.',
  alternates: {
    canonical: '/links',
  },
}
...
export default function LinksPage() {
  ...
  const avatarUrl = linksData.avatar.startsWith('/')
    ? linksData.avatar
    : `/assets/uploads/${linksData.avatar}`
        ...
        <div className="w-full flex flex-col gap-4 mb-12">
          {linksData.buttons.map((btn, idx) => {
            ...
            if (btn.type === 'whatsapp') {
              themeBorder =
                'hover:border-emerald-500/30 hover:shadow-[0_6px_20px_-8px_rgba(16,185,129,0.2)]'
              iconColorClass = 'text-emerald-400'
              bgLightClass = 'bg-emerald-500/10 border-emerald-500/20 group-hover:bg-emerald-500/15'
              iconContainer = (
                <div
                  className={`w-12 h-12 rounded-lg ${bgLightClass} border flex items-center justify-center flex-shrink-0 ${iconColorClass} transition-all duration-300`}
                >
              ...
              const imgUrl = btn.image
                ? btn.image.startsWith('/')
                  ? btn.image
                  : `/assets/uploads/${btn.image}`
                : null
            ...
            return (
              <LinkComponent
                ...
                href={btn.url}
                {...targetProps}
                className={`group w-full bg-[#121826]/75 p-4 rounded-xl flex items-center gap-4 text-left border border-white/5 ${themeBorder} hover:-translate-y-0.5 transition-all duration-300`}
              >
                {iconContainer}
                <div className="flex-grow">
                  <div
                    className={`text-white font-bold text-sm tracking-wide ${textHoverClass} transition-colors`}
                  >
                    {btn.title}
                  </div>
                  {btn.subtitle && (
                ...
                <div
                  className={`text-slate-600 ${arrowHoverClass} group-hover:translate-x-0.5 transition-all duration-300`}
                >
