          ...
          <FadeIn>
            <div className="mb-12">
              <h2 className="text-4xl font-bold mb-4 text-slate-900 dark:text-white">
                Etalase <span className="text-gradient">Produk Digital</span>
              </h2>
              <p className="text-slate-600 dark:text-slate-400 text-lg">
                Koleksi alat bantu, panduan, dan sistem siap pakai yang sedang dan telah dibangun.
              </p>
            </div>
          </FadeIn>
          ...
          <div className="flex flex-wrap justify-center gap-6">
            {items.map((item, index) => (
              <FadeIn
                key={item.slug}
                delay={0.1 * index}
                className="w-full sm:w-[calc(50%-12px)] lg:w-[calc(25%-18px)] flex flex-col"
              >
                <div className="glass-card rounded-2xl p-5 flex flex-col hover:border-purple-500/30 group h-full w-full">
                  ...
                  <div className="mb-4 flex-grow">
                    ...
                    <Link href={`/products/${item.slug}`}>
                      <h3 className="text-slate-900 dark:text-white font-bold mb-2 group-hover:text-cyan-400 transition-colors">
                        {item.name}
                      </h3>
                    </Link>
